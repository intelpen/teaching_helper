...
class VectorStoreWrapper:
    ...
    def __init__(self, config):
        self.embedding_function = HuggingFaceEmbeddings(model_name=os.environ.get('EMBEDDING_MODEL_PATH'))
        self.loader = DirectoryLoader('../res/pdfs/', glob="**/*.pdf", show_progress=True, use_multithreading=True,
                                      loader_cls=PyPDFLoader)
        self.collection_name = "collection-main"
        self.db_persist_directory = "../res/db"
    ...
    def load_default_docs(self):
        ...
        if os.path.exists('../res/db'):
            print("Embeddings for default documents already existent, skipping file upload..")
            return False
        else:
            print("\nloading documents..")
    ...
    def get_question_context(self, original_user_prompt):
        ...
        retrieved_docs = self.chromadb_instance.similarity_search_by_vector(query_embedding, k=2)
